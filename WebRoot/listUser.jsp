...
<a href="login.jsp"><button class="btn btn-success"> 退&nbsp;&nbsp;出</button></a>
  ...
  <div id="div3">
   ...
   <div id="div37">
   	<a href="/WarehouseManage//Add.do?select=good"><button class="btn btn-success"style="width:100%;height:95%;"> 添加商品</button></a>
   </div>
  ...
  <div id="div4">
   ...
   <table>
 <tr>
  <form method="post" action="/WarehouseManage//inquiry.do?select=user&keyword=Cno">
  <td><input type="text" name="Cno" placeholder="请在这里输入用户号"  class="form-control" style="width: 250px ;"></td>
   <td><button type="submit"class="btn btn-success"  name="submit"  style="width:60px;">确&nbsp;&nbsp;定</button></td>
   <td>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</td>
  </form>
  <form method="post" action="/WarehouseManage//inquiry.do?select=user&keyword=Cname" >
  <td><input type="text" name="Cname" placeholder="请在这里输入用户名称"  class="form-control" style="width: 250px ;"></td>
  <td> <button type="submit"class="btn btn-success"  name="submit"  style="width:60px;">确&nbsp;&nbsp;定</button></td>
  </form>
  </tr>
 </table>
  ...
  <div>
    <table class="table table-striped table-bordered table-hover table-condensed">
        ...
        <caption>用户列表 - 共${page.total}位</caption>
        ...
        <tbody>
        <c:forEach items="${users}" var="s" varStatus="status">
            <tr>
            <td width="95" height="95"><img src="${s.img }" width=100% height=100% /></td>
                ...
                <td style="text-align:center; vertical-align:middle;">${s.power}</td>
            	       
                 <td style="text-align:center; vertical-align:middle;">${s.remark}</td>
                  
                 <td style="text-align:center; vertical-align:middle;"><a href="/WarehouseManage/Update.do?select=power&option=${s.power }&id=${s.id}">变更权限<span class="glyphicon glyphicon-edit"></span> </a></td>
                <td style="text-align:center; vertical-align:middle;"><a href="/WarehouseManage/Delete.do?id=${s.id}&select=user" onclick="return Del();">删除<span class="glyphicon glyphicon-trash"></span> </a></td>
   ...
   <nav  style="margin:0 auto; width:300px ">
    <ul class="pagination">
        <li <c:if test="${!page.hasPreviouse}">class="disabled"</c:if>>
            <a href="/WarehouseManage/list.do?page.start=0&&select=user">
                <span>«</span>
            </a>
        </li>

        <li <c:if test="${!page.hasPreviouse}">class="disabled"</c:if>>
            <a href="/WarehouseManage/list.do?page.start=${page.start-page.count}&select=user">
                <span>‹</span>
            </a>
        </li>
        ...
        <c:forEach begin="0" end="${page.totalPage-1}" varStatus="status">
            ...
            <c:if test="${status.count*page.count-page.start<=30 && status.count*page.count-page.start>=-10}">
                <li <c:if test="${status.index*page.count==page.start}">class="disabled"</c:if>>
                    <a
                            href="/WarehouseManage/list.do?page.start=${status.index*page.count}&select=user"
        ...
        <li <c:if test="${!page.hasNext}">class="disabled"</c:if>>
            <a href="/WarehouseManage/list.do?page.start=${page.last}&select=user">
                <span>»</span>
            </a>
        </li>
    </ul>
