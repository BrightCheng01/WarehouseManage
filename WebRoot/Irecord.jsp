...
<a href="login.jsp"><button class="btn btn-success"> 退&nbsp;&nbsp;出</button></a>
  ...
  <div id="div4">
   ...
   <table class="table table-striped table-bordered table-hover table-condensed">
        ...
        <caption>进货记录列表 - 共${page.total}条</caption>
        ...
        <c:forEach items="${irecords}" var="s" varStatus="status">
            
        
            <tr>
            	<td style="text-align:center; vertical-align:middle;">${s.ino}</td>
                <td style="text-align:center; vertical-align:middle;">${s.iname}</td>
                <td style="text-align:center; vertical-align:middle;">${s.inum}</td>
                 <td style="text-align:center; vertical-align:middle;">${s.iprice}</td>
     
      
            	<td style="text-align:center; vertical-align:middle;">${s.totaliprice}</td>
            	
            	<td style="text-align:center; vertical-align:middle;">${s.iaddress}</td>
            	<td style="text-align:center; vertical-align:middle;">${s.idate}</td>
            	<td style="text-align:center; vertical-align:middle;">${s.operator}</td>
           		<td style="text-align:center; vertical-align:middle;">${s.iremark}</td>
   ...
   <nav  style="margin:0 auto; width:300px ">
    <ul class="pagination">
        <li <c:if test="${!page.hasPreviouse}">class="disabled"</c:if>>
            <a href="/WarehouseManage/ListRecord.do?page.start=0&&select=irecord">
                <span>«</span>
            </a>
        </li>

        <li <c:if test="${!page.hasPreviouse}">class="disabled"</c:if>>
            <a href="/WarehouseManage/ListRecord.do?page.start=${page.start-page.count}&select=irecord">
                <span>‹</span>
            </a>
        </li>
        ...
        <c:forEach begin="0" end="${page.totalPage-1}" varStatus="status">
            ...
            <c:if test="${status.count*page.count-page.start<=30 && status.count*page.count-page.start>=-10}">
                <li <c:if test="${status.index*page.count==page.start}">class="disabled"</c:if>>
                    <a
                            href="/WarehouseManage/ListRecord.do?page.start=${status.index*page.count}&select=irecord"
        ...
        <li <c:if test="${!page.hasNext}">class="disabled"</c:if>>
            <a href="/WarehouseManage/ListRecord.do?page.start=${page.last}&select=irecord">
                <span>»</span>
            </a>
        </li>
    </ul>
